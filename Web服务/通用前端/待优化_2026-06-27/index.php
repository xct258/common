...
if (strtolower(basename(rtrim($requestPath, '/'))) === 'favicon.ico') {
...
$config = [
    'folders' => [
        'mobile' => [
            'path' => dirname(__DIR__, 3) . '/images/pe/',
            'id_prefix' => 'pe-image',
        ],
        'desktop' => [
            'path' => dirname(__DIR__, 3) . '/images/pc/',
            'id_prefix' => 'pc-image',
        ],
    ],
    'mime_types' => [
...
function analyzeImage(string $imagePath, array $config): array
{
    ...
    if (isset($cache[$imagePath], $cache[$imagePath]['mtime'], $cache[$imagePath]['theme'])) {
        if ((int) $cache[$imagePath]['mtime'] === $mtime && (int) ($cache[$imagePath]['version'] ?? 0) === COLOR_CACHE_VERSION) {
...
function ensureImageMagickConvert(): void
{
    static $available = null;

    if ($available === null) {
        $available = function_exists('shell_exec') && trim((string) shell_exec('command -v convert 2>/dev/null')) !== '';
    }

    if (!$available) {
        respondJsonError('ImageMagick convert is required to extract image theme colors.', 500);
    }
}
